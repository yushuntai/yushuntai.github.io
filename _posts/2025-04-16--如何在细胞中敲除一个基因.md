...
（一）非同源末端连接（NHEJ, Non-Homologous End Joining） 
1.特性：此修复途径无需借助模板，直接将断裂的末端连接起来。其修复速度较快，然而却容易在连接过程中引入插入或缺失（Indels）片段，进而引发移码突变，最终达成基因敲除的效果。 
2.运作机制：断裂末端首先被 KU 蛋白复合物识别并加以保护，避免其进一步受损。随后核酸酶对断裂末端进行修剪，使其具备连接条件，最后由 DNA 连接酶 IV 完成重新连接的任务。 
3.应用范畴：在基因敲除以及功能缺失相关研究领域应用广泛。 （二）同源定向修复（HDR, Homology-Directed Repair） 1.特性：该修复途径必须依赖同源模板，这个模板既可以是外源供体 DNA，也可以是内源同源序列。依托模板进行修复，能够保障修复的精确性。不过，HDR 仅在细胞周期的 S/G2 期才会发生，因为此阶段细胞中有姐妹染色单体可充当修复模板。 2.运作机制：双链断裂产生，断裂末端会被切除，进而形成单链 DNA（ssDNA），这些单链 DNA 会与同源模板进行配对。之后依据模板序列合成新的 DNA 链，以此实现诸如点突变、基因插入等精准编辑操作。 3.应用范畴：主要应用于精确基因敲入以及定点修饰等方面。
