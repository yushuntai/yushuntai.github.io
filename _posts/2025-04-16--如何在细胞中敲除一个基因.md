...
### Cas9蛋白
Cas9属于核酸内切酶，扮演剪刀角色，它有两个domain具有切割功能。
**HNH domain：** 该domain专门切割与 sgRNA 互补的 DNA 链，此链也被称作靶链。
**RuvC domain：** 其作用对象为非互补链（非靶链）。Cas9 识别靶点的过程依赖于 PAM 序列（全称为 Protospacer Adjacent Motif，一般呈现为 NGG 形式）。
