...
1. pegRNA和Cas9-逆转录融合酶形成一个complex，在pegRNA引导下complex找到目的基因位置并结合上去，对目标DNA的两条DNA链（图中A和B链）。Cas9对B链PAM旁边的位置进行单链切割，造成切口，
2. 然后pegRNA上的PBS序列结合到目标DNA的B链上，然后逆转录酶在这里按照**想要编辑进基因组的模板**开始逆转录。
3. 然后在B链的编辑目标位置上，出现了两段DNA链，一段是带着编辑好的序列（带红色的部分），另一段是不带编辑序列的，这两段与A链竞争结合，如果是带编辑的结合到A链上，因为序列不匹配，会启动细胞内DNA错配修复机制，得到两条链都带有编辑的序列。但这种

<img src="https://github.com/user-attachments/assets/fe4dc96a-b78d-43bd-b80a-26c599c74a3c" alt="alt text" width="800">
