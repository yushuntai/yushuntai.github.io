...
右键——打印——打印机选成Foxit PDF Editor Printer——打印
...
```
library(pdftools)    #加载R包
setwd("D:/王凤超课题组/找工作/江西省/北京大学 生物学 生命科学学院 教学科研岗 余顺太 18810915690/6.科研材料证明文件/")   #设定工作路径，不要输入，用Tab键补齐
getwd()   #检验工作路径设定
pdf_file <- 'Single-cell genomic and transcriptomic landscapes of primary and metastatic colorectal cancer tumors.pdf' #该文件夹下要进行操作的文件名字，使用Tab键可以自动补齐，不用一个个输入字母。
pdf_subset(input = pdf_file, pages = 1)  # 提取第一页
