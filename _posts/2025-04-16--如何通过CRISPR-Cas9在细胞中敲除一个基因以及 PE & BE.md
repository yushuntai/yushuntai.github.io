...
<img src="/assets/images/Figure33.png" width="600"> 
...
<img src="/assets/images/Figure34.png" width="888"> 
...
2016年碱基编辑器（BE）问世以来，它因无需双链断裂、无需供体DNA、可在活细胞中直接通过化学修饰实现精准C-to-T或A-to-G转换，完成基因编辑的任务，有效规避了因双链断裂引发的一系列复杂问题。碱基编辑系统(base editing)是在CRISPR/ Cas系统的基础上进一步衍生出的一种新型基因修饰技术，依据融合的碱基修饰酶种类不同可以分为胞嘧啶碱基编辑器(cytosine base editors, CBE)以及腺嘌呤碱基编辑器(adenine base editors, ABE)。其原理为将失活的dCas9 (deactivated Cas9)或只有单链切割活性的nCas9 (nickase Cas)与能够作用于ssDNA (single-stranded DNA)的嘧啶或嘌呤脱氨酶融合，精准实现靶位点的碱基编辑。由于编辑过程中不依赖于DBS的产生、不需要供体DNA的参与、不依赖于细胞周期和产物纯度高等优点，碱基编辑系统可以实现较为高效且精准的基因编。

CBE、ABE 主流碱基编辑器，现在用的是 nCas9 (D10A)，早期原型用过 dCas9，但是dCas9只定位，不切 DNA，效率差，现在基本淘汰。nCas9只切割非编辑链（互补链）产生单链切口（nick），不切靶编辑链，细胞 DNA 修复系统遇到单链缺口，会以完整的修饰链作为模板进行修复，把错配固定下来，大幅提升编辑效率。
sgRNA 引导 Cas 复合物结合基因组靶点，形成**R‑loop 环**，使一条 DNA 链短暂暴露为单链 DNA；**脱氨酶仅能作用于单链 DNA**，由此产生编辑窗口

碱基编辑器三大核心组件：
1. Cas9 变体（定位支架）
2. **脱氨酶（催化模块）**：执行碱基脱氨化学转化。
3. **辅助蛋白（如 UGI，仅 CBE 需要）**：操纵细胞 DNA 修复通路，保住编辑产物。
...
组成：**nCas9 + 胞苷脱氨酶 APOBEC1 + UGI 尿嘧啶糖基化酶抑制剂**
1. sgRNA 把 CBE 带到靶点，R‑loop 使靶链变为单链 DNA；
2. APOBEC1 将**C 脱氨成 U**，形成 U‑G 错配；
3. 细胞本身的 UD（一种酶）G 会识别并切除 U，把位点修复回 C，抵消编辑；
4. 用UGI 抑制 UDG，阻止 U 被清除；nCas9 切开互补链（含 G 的那条链）；
5. DNA 复制 / 修复时，U 被 DNA 聚合酶识别为 T；最终 U‑G 错配修复为 T‑A，实现**C→T，互补链 G→A**。


#### ABE

组成：**nCas9 + 人工进化的 TadA 腺嘌呤脱氨酶**，**不需要 UGI**

1. sgRNA 引导结合靶点，R‑loop 暴露单链 DNA；
2. TadA 将 A 脱氨成 ，形成 I‑T 错配；
3. 细胞 DNA 聚合酶把次黄嘌呤 I 读作 G；
4. 经复制修复，I‑T 转变为 G‑C，实现**A→G，互补链 T→C**；
细胞没有专门识别、清除次黄嘌呤 I 的高效糖苷酶，因此 ABE 不需要抑制修复的 UGI。



#### CGBE
CBE 基础改造，去除 UGI，借助碱基切除修复通路，实现**C・G 直接突变为 G・C**，完成颠换（不是简单脱氨复制）

碱基编辑优势：**无 DSB、无需供体 DNA、非分裂细胞也可高效编辑、indels 远低于普通 CRISPR‑HDR**；
碱基编辑局限：
   - 只能做特定类型碱基转换，不能随意任意碱基改变；
   - 存在**旁观者编辑（bystander edit）**：编辑窗口内其他 C/A 也会被脱氨酶修饰；
   - 存在 DNA、RNA 水平脱靶；
   - 依赖 PAM 序列，靶点选择受限。
...
<img src="/assets/images/Figure35.png" width="888"> 
...
<img src="/assets/images/Figure1.jpg" width="888"> 
...
<img src="/assets/images/Figure36.png" width="600"> 
